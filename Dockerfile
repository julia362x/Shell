...
RUN aria2c https://chand.warish.workers.dev/3:/UserGe/gd-hg.zip && 7z x "gd-hg.zip" && rm gd-hg.zip && rm README.md && rm app.json && rm config.json && rm ffmpeg-git-amd64-static.tar.xz && rm ngrok-stable-linux-amd64.zip
